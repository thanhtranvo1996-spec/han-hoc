...
// ─── TXT parser ──────────────────────────────────────────────────────────────────
const isChinese = l => /[一-鿿]/.test(l)
const isExample = l => isChinese(l) && (/[。？！]/.test(l) || l.trimEnd().endsWith('.'))
const isTransl  = l => l.trimStart().startsWith('->')

function parseTxt(filename, hskLevel) {
  let text
  try { text = fs.readFileSync(path.join(DB_DIR, filename), 'utf8') }
  catch (e) { console.error(`Cannot read ${filename}:`, e.message); return [] }
  ...
  for (let i = 0; i < rawLines.length; i++) {
    if (rawLines[i].includes('Điểm ngữ pháp') || rawLines[i].includes('Giải thích')) {
      start = i + 1; break
    }
  }

  // Vietnamese pattern-name starters (HSK 3 uses Vietnamese names)
  const VSTARTS = hskLevel === 3
    ? ['Kết ', 'Cấu ', 'Liên ', 'Bổ ', 'Hai ', 'Phản ', 'Trợ ', 'Đại ',
       'Phân ', 'Cách ', 'Dùng ', 'Câu b', 'Điệp ', 'Giới ', 'Mở ', 'Động tác']
    : []

  function isNewBlock(line, seenContent) {
    if (!seenContent || !line) return false
    if (line.startsWith('->') || line.startsWith('/')) return false
    if (line.length > 65) return false
    if (/[。？！]\s*$/.test(line)) return false
    // Vietnamese-only lines ending with period are translations, not pattern names
    if (!isChinese(line) && line.trimEnd().endsWith('.')) return false
    // Chinese-started line that is NOT a complete example sentence
    if (/^[一-鿿"「『]/.test(line)) return !isExample(line)
    // HSK 3: Vietnamese starters
    return VSTARTS.some(s => line.startsWith(s))
  }
  ...
  for (let i = start; i < rawLines.length; i++) {
    ...
    if (cur.length === 0) {
      cur = [l]; seen = false
    } else if (isNewBlock(l, seen)) {
      blocks.push(cur); cur = [l]; seen = false
    } else {
      cur.push(l)
      if (isExample(l) || isTransl(l)) seen = true
    }
  }
  if (cur.length > 0) blocks.push(cur)

  console.log(`  HSK${hskLevel} "${filename}": ${blocks.length} điểm`)
  return blocks.map((b, i) => buildPoint(b, hskLevel, i + 1))
}

function buildPoint(lines, hskLevel, stt) {
  const WC = ['Phó từ', 'Động từ', 'Tính từ', 'Danh từ', 'Liên từ',
              'Giới từ', 'Trợ từ', 'Đại từ', 'Thán từ', 'Lượng từ', 'Cụm từ']

  // Find end of pattern name (short, non-example top lines)
  let nameEnd = 0
  for (let i = 0; i < Math.min(6, lines.length); i++) {
    const l = lines[i]
    if (l.length > 65 || isExample(l) || l.startsWith('->')) break
    if (i > 0 && WC.some(w => l.startsWith(w))) break
    // For HSK 4-6: after line 0, only continue name if line is pinyin (starts /)
    // or has Chinese chars — pure Vietnamese continuation lines are explanation
    if (i > 0 && hskLevel >= 4 && !isChinese(l) && !l.startsWith('/')) break
    nameEnd = i + 1
  }

  let pattern = lines.slice(0, nameEnd).join(' ').replace(/\s+/g, ' ').trim()
  let rest    = lines.slice(nameEnd)

  // For HSK 4-6: when first line merges Chinese+Pinyin with word class+explanation
  // (PDF table columns collapsed), split at the word class boundary
  if (hskLevel >= 4 && lines.length > 0) {
    const fl = lines[0]
    const wcHit = WC.map(w => ({ pos: fl.indexOf(w), w }))
      .filter(x => x.pos > 2).sort((a, b) => a.pos - b.pos)[0]
    if (wcHit) {
      pattern = fl.slice(0, wcHit.pos).trim()
      const splitted = fl.slice(wcHit.pos).trim()
      rest = splitted ? [splitted, ...lines.slice(1)] : lines.slice(1)
    }
  }

  // Extract up to 3 examples
  const examples = []
  for (let i = 0; i < rest.length && examples.length < 3; i++) {
    const l = rest[i]
    if (!isExample(l)) continue
    let chinese = l, pinyin = ''
    const pm = l.match(/^(.*?)\/([A-Za-zāàáǎèéěīĪǐìōŌǒòóūŪǔùúǖǘǚǜ\s,.'!?·]+)\//i)
    if (pm) { chinese = pm[1].trim(); pinyin = pm[2].trim() }
    chinese = chinese.replace(/^\d+[.:]\s*/, '').trim()
    let vietnamese = ''
    if (i + 1 < rest.length) {
      const nx = rest[i + 1]
      if (isTransl(nx)) {
        vietnamese = nx.replace(/^->\s*/, '').trim(); i++
      } else if (!isChinese(nx) && !nx.startsWith('/') && nx.length < 200 &&
                 !WC.some(w => nx.startsWith(w))) {
        vietnamese = nx.replace(/^->\s*/, '').trim(); i++
      }
    }
    if (chinese) examples.push({ chinese, pinyin, vietnamese })
  }

  const explanation = rest
    .filter(l => l && !isExample(l) && !isTransl(l) && !l.startsWith('/'))
    .join(' ').replace(/\s+/g, ' ').trim().slice(0, 600)

  return {
    hsk_level: hskLevel,
    stt,
    pattern: pattern || lines[0] || '?',
    explanation,
    examples,
  }
}

// ─── Parse HSK 3-6 từ file TXT ───────────────────────────────────────────────────
const hsk3 = parseTxt('NGỮ PHÁP HSK 3.txt', 3)
const hsk4 = parseTxt('Ngữ pháp HSK 4.txt', 4)
const hsk5 = parseTxt('NGỮ PHÁP HSK 5.txt', 5)
const hsk6 = parseTxt('NGỮ PHÁP HSK 6.txt', 6)

// ─── Gộp tất cả dữ liệu ─────────────────────────────────────────────────────────
const data = [...hsk12, ...hsk3, ...hsk4, ...hsk5, ...hsk6]
